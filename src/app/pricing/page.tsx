...
function FeatureValue({ value }: { value: boolean | string }) {
  if (value === false) {
    return <span className="text-white/40 text-lg font-black">—</span>
  }
  if (value === true) {
    return (
      <div className="w-6 h-6 rounded-full bg-white/10 flex items-center justify-center mx-auto">
  ...
  return (
    <div className="relative bg-[#060606] min-h-screen text-white selection:bg-primary/30 antialiased overflow-hidden font-sans">
      ...
      <section className="relative pt-40 pb-16 px-6 max-w-7xl mx-auto text-center z-10">
        ...
        >
          FAIR. EINFACH.{' '}
          <span className="text-white">DEIN PREIS.</span>
        </motion.h1>
        ...
        <motion.p
          ...
          className="text-lg text-white max-w-xl mx-auto mb-4"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {tiers.map(([key, tier], i) => {
            ...
            return (
              ...
              >
                {isMax && (
                  <div className="absolute top-0 right-0 bg-white text-black text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-bl-2xl rounded-tr-[2rem]">
                    Bestseller
                  </div>
                )}
                {isPopular && (
                  <div className="absolute top-0 right-0 bg-white/20 text-white text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-bl-2xl rounded-tr-[2rem]">
                    Beliebt
                  </div>
                )}

                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${isMax ? 'bg-white/20 border border-white/20' : 'bg-white/5 border border-white/10'}`}>
                  <Icon className={`w-6 h-6 ${isMax ? 'text-white' : 'text-white/80'}`} />
                </div>

                <div className="flex items-center gap-2 mb-2">
                  <h2 className="text-2xl font-black tracking-tight">{tier.name}</h2>
                  {key === 'starter' && (
                    <span className="text-[10px] font-bold bg-white/10 text-white px-2 py-0.5 rounded-full uppercase tracking-wider">
                      Das Zuckerl
                    </span>
                  )}
                </div>

                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-widest text-white mb-6 w-fit">
                ...
                <div className="flex items-baseline gap-1 mb-8">
                  {tier.price === 0 ? (
                    ...
                    <>
                      ...
                      <span className="text-white/60 font-medium">/Monat</span>
                    </>
                  )}
                </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left px-6 py-5 text-white font-bold uppercase tracking-widest text-[10px] w-1/2">Feature</th>
                  {tiers.map(([key, tier]) => (
              ...
              <tbody className="divide-y divide-white/5">
                {FEATURES_MATRIX.map((feature, i) => (
                  <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                    <td className="px-6 py-4 text-white font-medium">{feature.label}</td>
                    {tiers.map(([key]) => (
